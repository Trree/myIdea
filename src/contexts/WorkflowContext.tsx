...
export function WorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const skipCurrentStep = useCallback(() => {
    setState((prev) => {
      if (!canSkipCurrentStep(prev)) {
        console.warn('Cannot skip current step');
        return prev;
      }
  ...
  const canSkipStep = useCallback(() => canSkipCurrentStep(state), [state]);

  const canContinueToNext = useCallback(() => canContinue(state), [state]);

  const canGoBackToPrev = useCallback(() => canGoBack(state), [state]);
